...
import { Play, Coins, Gem, Home, User, ShoppingBag, Settings, LogIn, UserPlus, Users, Database } from "lucide-react";
...
export function TopBar({
  ...
  return (
    <header className="flex h-16 shrink-0 items-center justify-between border-b border-white/10 bg-[#0a120e]/75 backdrop-blur-md px-4 sm:px-6 z-30 select-none shadow-lg relative">
      {/* Sol: Logo */}
      ...
      {/* Orta: Navigasyon Sekmeleri (Esnek ve Çakışmayan Düzen) */}
          ...
          {navItems.map((item) => {
            ...
            if (item.isSpecial) {
              return (
                <button
                  ...
                  className={`relative group mx-1 sm:mx-2 flex items-center gap-1.5 sm:gap-2 px-4 sm:px-6 lg:px-7 py-2 sm:py-2.5 rounded-xl font-black text-[11px] sm:text-xs tracking-widest uppercase transition-all overflow-hidden cursor-pointer shrink-0 ${
                    isSelected
            ...
            return (
              <button
                key={item.id}
                onClick={() => onTabChange(item.id)}
                className={`flex items-center gap-1 sm:gap-1.5 rounded-xl px-2 sm:px-3 lg:px-4 py-1.5 sm:py-2 text-[11px] sm:text-xs tracking-wider transition-all uppercase cursor-pointer shrink-0 ${
                  isSelected
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 shrink-0" suppressHydrationWarning>
        {isLoading && !user ? (
          ...
          <>
            {/* Düz Coin Bakiyesi (Geniş ekranlarda gösterilir) */}
            <div
              onClick={() => onTabChange("store")}
              className="hidden xl:flex items-center gap-2 px-3 py-1.5 rounded-xl border border-white/10 bg-black/40 shadow-2xs hover:border-amber-400/50 hover:bg-amber-500/10 transition-all cursor-pointer group"
        ...
        {onToggleSocial && (
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border transition-all cursor-pointer shadow-2xs active:scale-95 ${
              isSocialOpen
                ? "bg-[#15803d] text-white border-emerald-400 shadow-md shadow-emerald-950/50"
                : "border-white/15 bg-black/40 text-zinc-300 hover:border-emerald-500/40 hover:text-white hover:bg-white/5"
            }`}
            title="Sosyal Panel & Arkadaşlar"
