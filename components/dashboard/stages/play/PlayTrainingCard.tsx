        ...
        {TRAINING_SUB_MODES.map((sub) => {
          ...
          if (!sub.isAvailable) {
            return (
              <div
                key={sub.id}
                className="w-full h-[44px] flex items-center justify-between px-3.5 rounded-xl border border-white/5 bg-black/20 text-zinc-500 select-none cursor-not-allowed"
              >
