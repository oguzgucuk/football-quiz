...
import { CheckCircle2, Search, Lock, AlertCircle } from "lucide-react";
...
export function TeamPicker({
  ...
  const handlePick = (team: Team) => {
    if (usedTeamIds.includes(team.id)) {
      setErrorMessage(`"${team.name}" bu maç oturumunda daha önce seçildi!`);
              ...
              {suggestions.map((team, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    {isUsed ? (
                      <span className="text-[11px] font-semibold px-2 py-0.5 rounded-md bg-amber-500/15 text-amber-300/90 border border-amber-500/20 flex items-center gap-1">
